...
function updateTextContent(lang) {
    ...
    // Update linked images section titles and descriptions
    for (let i = 1; i <= 6; i++) {
        const linkTitle = document.querySelector(`.link-title-${i}`);
        if (linkTitle) {
            linkTitle.textContent = linkTitle.getAttribute('data-' + lang);
        }
        
        const linkDesc = document.querySelector(`.link-desc-${i}`);
        if (linkDesc) {
            linkDesc.textContent = linkDesc.getAttribute('data-' + lang);
        }
    }
